    ...
    $count  = $stmt->rowCount();
        ...
        if($json==true){
            if ($count > 0 ) {
                ...
                echo json_encode(array("status" => "success", "data" => $data,"message"=>$successmess));
            } else {
...
function   result($count,$successmessage,$failmessage){
    if($count>0){
        printSuccess($successmessage);
    }else{
        printFailure($failmessage);
    }
}

function sendEmail($to , $title , $body){
    $header = "From: AbdallaAshraf@support " . "\n" . "CC: abdallahashraf2001b@gmail.com" ; 
